"""
monoseme_analysis.py
==============================

This script identifies and compares the most dissimilar pairs of monosemes in two Word2Vec embedding spaces:
TinyStories and RecipeNLG. It includes both total word frequencies and the number of distinct documents
each word appears in, providing richer information for analyzing semantic distances.
...
5. Compute all pairwise cosine distances and select the 1000 most dissimilar pairs
...
Output:
-------
- Two CSV files listing the 1000 most dissimilar monoseme pairs:
  - base_tiny_dissimilar_pairs.csv
  - base_recipe_dissimilar_pairs.csv

- Two summary .txt files with corpus statistics:
  - base_tiny_dissimilar_pairs_summary.txt
  - base_recipe_dissimilar_pairs_summary.txt

Usage:
------
Run the script directly:
$ python monoseme_analysis.py
"""
...
WORDLIST_CSV_PATH = '../data/all_words.csv'  # Beehuizen's wordlist (word,label format)
MONOSEME_TXT_PATH = 'monosemes_only.txt'     # Output: plain text list of monosemes (one per line)

# TinyStories inputs
TINY_MODEL_PATH = '/home/nina/ambiguity/word2vec_pipelines/word2vec_base_tiny/word2vec.model'
TINY_CORPUS_PATH = '/home/nina/ambiguity/word2vec_pipelines/word2vec_base_tiny/preprocessed_corpus.json'
TINY_CSV_PATH = 'base_tiny_dissimilar_pairs.csv'
...
RECIPE_CSV_PATH = 'base_recipe_dissimilar_pairs.csv'
...
def compute_dissimilar_pairs(model, words, doc_freqs, max_ratio=MAX_FREQ_RATIO, top_n=TOP_N):
...
def print_results(title, results):
    """Prints the top 10 most dissimilar word pairs for quick inspection."""
    print(f"\n=== {title} ===")
...
def main():
    ...
    tiny_results = compute_dissimilar_pairs(model_tiny, monosemes_tiny, doc_freqs_tiny, top_n=TOP_N)
    save_extended_results(tiny_results, model_tiny, doc_freqs_tiny, TINY_CSV_PATH)
    print_results("TinyStories - Most Dissimilar Monoseme Pairs", tiny_results)
    ...
    recipe_results = compute_dissimilar_pairs(model_recipe, monosemes_recipe, doc_freqs_recipe, top_n=TOP_N)
    save_extended_results(recipe_results, model_recipe, doc_freqs_recipe, RECIPE_CSV_PATH)
    print_results("RecipeNLG - Most Dissimilar Monoseme Pairs", recipe_results)

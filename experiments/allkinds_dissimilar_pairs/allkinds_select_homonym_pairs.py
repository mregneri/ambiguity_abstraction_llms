...
This script selects a set of most dissimilar non-overlapping word pairs from CSV files
created by `allkinds_homonym_analysis.py`, ensuring that each word appears in only
one pair.

These pairs are intended for the creation of artificial homonyms in the
...
also creates a second output file per corpus containing only the top 55 most dissimilar
pairs (sorted by cosine similarity, lowest first).
...
- Finally, sort the selected pairs by similarity (ascending) and keep only the top 55
  most dissimilar pairs for the additional trimmed output files.
...
--------
TinyStories:
  - homonym_pairs_tiny_allkinds.csv         (all selected non-overlapping pairs)
  - homonym_pairs_tiny_allkinds_top55.csv   (top 55 most dissimilar by cosine similarity)

RecipeNLG:
  - homonym_pairs_recipe_allkinds.csv       (all selected non-overlapping pairs)
  - homonym_pairs_recipe_allkinds_top55.csv (top 55 most dissimilar by cosine similarity)

Usage:
------
$ python allkinds_select_homonym_pairs.py
"""
...
TRIM_TO = 55  # Number of most dissimilar pairs to keep in the "top" files

PAIR_FILES = [
    {
        'name': 'TinyStories (all kinds)',
        'input_csv': 'tiny_base_dissimilar_pairs_allkinds.csv',
        'output_csv_full': 'homonym_pairs_tiny_allkinds.csv',
        'output_csv_top': 'homonym_pairs_tiny_allkinds_top55.csv',
    },
    {
        'name': 'RecipeNLG (all kinds)',
        'input_csv': 'recipe_base_dissimilar_pairs_allkinds.csv',
        'output_csv_full': 'homonym_pairs_recipe_allkinds.csv',
        'output_csv_top': 'homonym_pairs_recipe_allkinds_top55.csv',
    }
]
...
def select_pairs(pairs):
    ...
    For each word (in order of increasing pair count), we choose the pair with the
    lowest cosine similarity among those where both words are still unused.
    """
...
def trim_pairs_by_similarity(pairs, max_pairs):
    """
    Sort pairs by cosine_similarity (ascending) and keep only the top `max_pairs`
    most dissimilar pairs (lowest similarity values).
    ...
    sorted_pairs = sorted(pairs, key=lambda p: p['cosine_similarity'])
...
def main():
    for config in PAIR_FILES:
        ...
        print("Selecting non-overlapping homonym candidate pairs (all kinds)...")
        ...
        # Create and save top-N subset sorted by dissimilarity (lowest similarity)
        top_path = config['output_csv_top']
        top_pairs, _ = trim_pairs_by_similarity(selected, TRIM_TO)
        print(f"Saving top {len(top_pairs)} most dissimilar pairs to: {top_path}")

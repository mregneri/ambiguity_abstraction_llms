"""
insert_artificial_hypernyms.py
=============================

This script inserts artificial hypernyms into tokenized training, validation, and test corpora.
...
Usage:
------
python insert_artificial_hypernyms.py --config config/insert_recipe_1_HYP.py
...
def build_inflection_map_from_master(master_map, replacements_map):
    """
    Build an inflection lookup map from the master inflection data and a replacements map.

    Returns a dict: {form.lower(): (base, artificial)}
    """
    inflection_map = {}
...
def save_replacement_log(train_repl, val_repl, test_repl, replacements_map,
    ...
    """Save a detailed grouped summary log of replacements and inflected forms."""
    os.makedirs(os.path.dirname(log_filename), exist_ok=True)
    with open(log_filename, 'w', encoding='utf-8') as f:
        f.write("Hypernym Replacement Summary Log\n")
        f.write("===============================\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Insert artificial hypernyms into corpora using master inflection maps."
    )

"""
generate_hypernym_configs.py
=============================

Generates config files for inserting artificial hypernyms into tokenized corpora.

Each config references the base corpora (train, val, test), output corpora,
the hypernym map, the verified master inflection map, and the summary log path.
"""
...
logs_base_dir = "/home/nina/ambiguity/experiments/create_hypernym_corpora/allkinds/logs"
...
def make_config(dataset, count):
    name = f"{dataset}_{count}_HYP"

    # Choose the correct master map for hypernyms
    master_inflections = (
        f"{master_inflections_dir}/{dataset}_master_HYP_inflections_edited.json"
    )
...
def main():
    ...
    print("\n🎉 All hypernym config files generated!")

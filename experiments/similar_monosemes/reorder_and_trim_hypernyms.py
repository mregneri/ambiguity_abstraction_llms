"""
reorder_and_trim_hypernyms.py
==============================

This script processes monoseme word pairs selected for experiments with artificial hypernyms.
It ensures that each pair is ordered by word frequency, removes any pair containing the word
"farmers", trims the list to a specified number of most similar pairs (based on cosine similarity),
checks whether the constructed artificial hypernym names appear in the corresponding corpus,
and finally produces a shuffled version of the list with the most similar pair at the top.

Main Features:
--------------
- Reorders each monoseme pair so the more frequent word comes first.
- Excludes pairs that contain the word "farmers".
- Keeps only the top N most similar pairs according to cosine similarity.
- Verifies whether each hypernym name exists in the target corpus.
- Saves the final selected pairs and a summary of removed and excluded pairs.
- Saves a shuffled version of the final pairs with the top-most similar pair at the top.
...
- Text summary files listing removed and excluded pairs and corpus name checks.
- CSV files with rows shuffled (keeping most similar pair first).

Usage:
------
$ python reorder_and_trim_hypernyms.py
"""
...
RECIPE_FILE = "recipe_hypernym_names.csv"
TINY_FILE = "tiny_hypernym_names.csv"
RECIPE_OUTPUT_FILE_SORTED = "recipe_trimmed_hypernyms_sorted.csv"
TINY_OUTPUT_FILE_SORTED = "tiny_trimmed_hypernyms_sorted.csv"
RECIPE_OUTPUT_FILE_SHUFFLED = "recipe_final_hypernyms_shuffled.csv"
TINY_OUTPUT_FILE_SHUFFLED = "tiny_final_hypernyms_shuffled.csv"
...
if __name__ == "__main__":
    process_dataset(RECIPE_FILE, RECIPE_OUTPUT_FILE_SORTED, RECIPE_CORPUS_PATH, RECIPE_OUTPUT_FILE_SHUFFLED)
    process_dataset(TINY_FILE, TINY_OUTPUT_FILE_SORTED, TINY_CORPUS_PATH, TINY_OUTPUT_FILE_SHUFFLED)
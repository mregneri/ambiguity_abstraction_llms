"""
allkinds_hypernym_analysis.py
==============================

This script identifies and compares the most similar pairs of words (of all types:
...
constructing artificial hypernyms in the "all word kinds" experiment.

This is a parallel pipeline to `monoseme_hypernym_analysis.py` and does NOT
modify or overwrite its outputs.
...
Output:
-------
- Two CSV files listing the 1000 most similar pairs (all word types):
  - tiny_base_similar_pairs_allkinds.csv
  - recipe_base_similar_pairs_allkinds.csv

- Two summary .txt files with corpus statistics:
  - tiny_base_similar_pairs_allkinds_summary.txt
  - recipe_base_similar_pairs_allkinds_summary.txt

Usage:
------
Run the script directly from its folder:
    $ python allkinds_hypernym_analysis.py
"""
...
TINY_CSV_PATH = 'tiny_base_similar_pairs_allkinds.csv'
...
RECIPE_CSV_PATH = 'recipe_base_similar_pairs_allkinds.csv'
...
def compute_similar_pairs(model, words, max_ratio=MAX_FREQ_RATIO, top_n=TOP_N):
    """
    ...
    list[tuple[(str, str), float]]
        List of ((word1, word2), similarity) tuples, sorted by decreasing similarity,
        truncated to top_n.
    """
...
def main():
    ...
    tiny_results = compute_similar_pairs(model_tiny, words_tiny, max_ratio=MAX_FREQ_RATIO, top_n=TOP_N)
    save_extended_results(tiny_results, model_tiny, doc_freqs_tiny, TINY_CSV_PATH)
    print_results("TinyStories - Most Similar Pairs (all word kinds)", tiny_results)
    write_summary("TinyStories", len(words_all), len(words_tiny),
                  tiny_results, TINY_CSV_PATH, MIN_DOC_FREQ_TINY)
    ...
    recipe_results = compute_similar_pairs(model_recipe, words_recipe, max_ratio=MAX_FREQ_RATIO, top_n=TOP_N)
    save_extended_results(recipe_results, model_recipe, doc_freqs_recipe, RECIPE_CSV_PATH)
    print_results("RecipeNLG - Most Similar Pairs (all word kinds)", recipe_results)
    write_summary("RecipeNLG", len(words_all), len(words_recipe),
                  recipe_results, RECIPE_CSV_PATH, MIN_DOC_FREQ_RECIPE)

"""
shuffle_and_check_hypernyms.py
==============================

This script performs the final preparation step for experiments with artificial
hypernyms. It takes the manually named top-50 hypernym pairs for each corpus
and:

1. Checks, for each hypernym name, whether it already occurs in the respective
   ...
   - Hypernym names from the CSV are stripped and lowercased before comparison.

2. Produces a shuffled version of the pairs where:
   - The first row (typically the most similar pair) remains in place.
   - All remaining rows are randomly shuffled in a reproducible way.

For each corpus (TinyStories, RecipeNLG), the script:

- Reads the corresponding `*_top50_hypernym_pairs_with_names.csv` file.
- Loads the preprocessed corpus and builds a normalized set of word tokens.
- Writes a summary text file listing, for every hypernym name, whether it was
...
Inputs:
-------
- tiny_top50_hypernym_pairs_with_names.csv
- recipe_top50_hypernym_pairs_with_names.csv
...
TinyStories:
  ...
  - tiny_hypernym_pairs_final_shuffled_summary.txt

RecipeNLG:
  - recipe_hypernym_pairs_final_shuffled.csv
  - recipe_hypernym_pairs_final_shuffled_summary.txt

Usage:
------
$ python shuffle_and_check_hypernyms.py
"""
...
TINY_INPUT = "tiny_top50_hypernym_pairs_with_names.csv"
RECIPE_INPUT = "recipe_top50_hypernym_pairs_with_names.csv"

TINY_OUTPUT = "tiny_hypernym_pairs_final_shuffled.csv"
RECIPE_OUTPUT = "recipe_hypernym_pairs_final_shuffled.csv"
...
def write_summary(pairs, corpus_words, dataset_name, input_csv_path, output_csv_path):
    """
    Write a summary text file listing, for each hypernym name, whether it is
    found in the corpus.
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write(f"Hypernym Name Corpus Check – {dataset_name}\n")
        f.write("=" * (len(dataset_name) + 29) + "\n\n")
...
def process_dataset(config):
    ...
    2. Load the named hypernym pairs from the input CSV.
    3. Write a summary indicating whether each hypernym name is found in the corpus.

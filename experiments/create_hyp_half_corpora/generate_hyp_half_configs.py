"""
generate_hyp_half_configs.py
=============================

Generates config files for inserting artificial hypernyms (50% replacements)
...
- The corresponding hypernym replacement map
- The master inflection map (shared for all HYP corpora)
- The path for the output summary log

Usage:
------
python generate_hyp_half_configs.py
"""
...
# Use allkinds hypernym maps from create_hypernym_corpora
maps_base_dir = "/home/nina/ambiguity/experiments/create_hypernym_corpora/allkinds/hypernym_maps"
logs_base_dir = "/home/nina/ambiguity/experiments/create_hyp_half_corpora/allkinds/logs"
...
master_inflections = {
    "recipe": f"{master_inflections_dir}/recipe_master_HYP_inflections_edited.json",
    "tiny": f"{master_inflections_dir}/tiny_master_HYP_inflections_edited.json",
}


def make_config(dataset, count):
    """Create one config file for a given dataset and count."""
    name = f"{dataset}_{count}_HYP_HALF"
...
config = {{
    ...
    'replacements': '{maps_base_dir}/{dataset}_{count}_HYP_map.json',
...
def main():
    ...
    print("\n🎉 All HYP_HALF config files generated!")
